...
export const GetTagsOfArticle = async (articleId: string) : Promise<Array<Tag>> => {
...
export const AddViewCount = async (articleId: string) => {
  let currentCnt = await DBConfig("ARTICLE").where({ArticleId: articleId}).select("ViewCount").first();
  if (!currentCnt)
    return;
  await  DBConfig("ARTICLE").where({ArticleId: articleId}).update({"ViewCount": currentCnt.ViewCount++});
}

export const GetRelativeArticle = async (categoryId: string, articleId: string) => {
  return DBConfig("ARTICLE")
      .join("ARTICLE_SUBCATEGORY",'ARTICLE_SUBCATEGORY.ArticleID', '=', 'ARTICLE.ArticleID')
...
export const UpdateBackgroundImageOfArticle = async (articleId: string, value: string = "null") => {
  return DBConfig("ARTICLE_URL").where({ArticleID: articleId, STT: 0}).update({URL: value});
}

export const AddBackgroundImageOfArticle = async (articleId: number, value: string = "null") => {
  return DBConfig("ARTICLE_URL").insert({ArticleID: articleId, STT: 0, URL: value});
}
...
export const GetCommentOfArticle = async (articleId: string): Promise<Array<Comment>> => {
  let comment = await DBConfig("COMMENT").where({
    ArticleId: articleId
  }).orderBy("DatePosted", "desc");
  return comment ? comment : [];
}

export const GetBackgroundImageOfArticle = async (articleId: string): Promise<string> => {
  let bgURL = await DBConfig("ARTICLE_URL").where({STT: 0, ArticleID: articleId}).first();
  return (bgURL && bgURL.URL) ? bgURL.URL : "null";
}

export const AddComment = async (articleId: string, content: string, date: Date) => {
