...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.form-group').forEach(element => {
        if (
            ...
            element.querySelector('select').id.includes('roleSelect_')
        ) {
            const id = element
                .querySelector('select')
                .id.replace('roleSelect_', '');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.actionBtn').forEach(button => {
        button.addEventListener('click', event => {
            ...
            const existingItem = resultContainer.querySelector(
            ...
            const resultHiddenContainer = document.getElementById(
                `hiddenCategory${id}`
            );

            const existingHiddenItem = resultHiddenContainer.querySelector(
                `[value="${selectedValue}"]`
            );
...
function removeCategoryItem(element) {
    ...
    const id = element
        .closest('.categoryInputResult')
        .id.replace('categoryResult_', '');
